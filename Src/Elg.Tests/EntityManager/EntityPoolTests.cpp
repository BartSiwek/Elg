...
TEST(EntityPool_DefaultState) {
  ...
  CHECK_EQUAL(static_cast<elg_uint32>(0), pool.GetActiveEntityCount());

  // Check that IDs are not active (we use the knowledge of internals here)
  for (elg_uint32 id = 0;id < 128;++id) {
...
TEST(EntityPool_UniqueIds) {
  ...
  // Execute & check
  for (elg_uint32 current_id_index = 0;current_id_index < 128;++current_id_index) {
    // Create a new ID
    elg_uint32 new_id = pool.CreateEntity();

    // Check if it differs from prev. ones
    for (elg_uint32 prev_id_index = 0;prev_id_index < current_count;++prev_id_index) {
...
TEST(EntityPool_SimpleCreate) {
  ...
  // Execute
  elg_uint32 entity_id = pool.CreateEntity();

  // Check that count is equal to zero and ids match
  CHECK_EQUAL(static_cast<elg_uint32>(1), pool.GetActiveEntityCount());
...
TEST(EntityPool_SimpleCreateDelete) {
  ...
  // Execute
  elg_uint32 entity_id = pool.CreateEntity();
  elg_uint32 entity_id_index = pool.GetEntityIndex(entity_id);
...
TEST(EntityPool_FillCreate) {
  ...
  // Execute
  elg_uint32 created_ids[TestEntityPoolType::MaxEntityCount];
  for (elg_uint32 current_id_index = 0;current_id_index < TestEntityPoolType::MaxEntityCount;++current_id_index) {
  ...
  const elg_uint32* active_entities = pool.GetActiveEntities();
  for (elg_uint32 current_id_index = 0;current_id_index < TestEntityPoolType::MaxEntityCount;++current_id_index) {
...
TEST(EntityPool_FillCreateDelete) {
  ...
  for (elg_uint32 current_id_index = 0;current_id_index < TestEntityPoolType::MaxEntityCount;++current_id_index) {
    elg_uint32 current_entity_id_index = pool.GetEntityIndex(created_ids[current_id_index]);
...
TEST(EntityPool_FillCreateDeleteRandom) {
  ...
  for (elg_uint32 interation_index = 0;interation_index < TestEntityPoolType::MaxEntityCount;++interation_index) {
    // Execute - create ids
    elg_uint32 created_ids[TestEntityPoolType::MaxEntityCount];
    for (elg_uint32 current_id_index = 0;current_id_index < TestEntityPoolType::MaxEntityCount;++current_id_index) {
      created_ids[current_id_index] = pool.CreateEntity();
    ...
    for (elg_uint32 current_count = TestEntityPoolType::MaxEntityCount;current_count > 0;--current_count) {
      // The rand is thread safe on VS according to MSDN
      elg_uint32 value = (rand() % current_count);  // NOLINT(runtime/threadsafe_fn)

      elg_uint32 to_be_deleted = created_ids[value];
      created_ids[value] = created_ids[current_count-1];
      created_ids[current_count-1] = to_be_deleted;

      elg_uint32 to_be_deleted_index = pool.GetEntityIndex(to_be_deleted);
      pool.DestroyEntity(to_be_deleted_index);
    }

    // Check that count is equal to zero and entity_ids are not active
    CHECK_EQUAL(static_cast<elg_uint32>(0), pool.GetActiveEntityCount());
    for (elg_uint32 current_id_index = 0;current_id_index < 10;++current_id_index) {
      CHECK_EQUAL(ELG_ENTITY_MANAGER_INVALID_ID, pool.GetEntityIndex(created_ids[current_id_index]));
    }
  }
}
...
TEST(EntityPool_RemoveingTopElement) {
  ...
  // Execute
  elg_uint32 ids[3];
  for (elg_uint32 current_id_index = 0;current_id_index < 3;++current_id_index) {
    ids[current_id_index] = pool.CreateEntity();
  }

  CHECK_EQUAL(static_cast<elg_uint32>(3), pool.GetActiveEntityCount());
  ...
  elg_uint32 id_1_index = pool.GetEntityIndex(ids[1]);
  pool.DestroyEntity(id_1_index);
  elg_uint32 id_2_index = pool.GetEntityIndex(ids[2]);
  pool.DestroyEntity(id_2_index);

  ids[1] = pool.CreateEntity();

  // Check
  CHECK_EQUAL(static_cast<elg_uint32>(2), pool.GetActiveEntityCount());
  CHECK_EQUAL(ids[0], pool.GetActiveEntities()[0]);
  CHECK(pool.GetEntityIndex(ids[0]) != ELG_ENTITY_MANAGER_INVALID_ID);
  CHECK_EQUAL(ids[1], pool.GetActiveEntities()[1]);
  CHECK(pool.GetEntityIndex(ids[1]) != ELG_ENTITY_MANAGER_INVALID_ID);
...
TEST(EntityPool_RandomOperations) {
    ...
    TestEntityPoolType pool;
    elg_uint32 current_count = 0;
    elg_uint32 ids[TestEntityPoolType::MaxEntityCount];
    for (elg_uint32 current_index = 0;current_index < TestEntityPoolType::MaxEntityCount;++current_index) {
    ...
    for (elg_uint32 operation_index = 0;operation_index < 100000;++operation_index) {
      // The rand is thread safe on VS according to MSDN
      elg_uint32 value = (rand() % TestEntityPoolType::MaxEntityCount);  // NOLINT(runtime/threadsafe_fn)
      if (value < current_count) {
        // Delete thing at value
        elg_uint32 to_be_deleted = ids[value];
        ...
        // Create one
        elg_uint32 created = pool.CreateEntity();
        ids[current_count++] = created;
      }

      // Do checking
      CHECK_EQUAL(pool.GetActiveEntityCount(), current_count);
      const elg_uint32* current_active = pool.GetActiveEntities();
      for (elg_uint32 current_active_index = 0;current_active_index < current_count;++current_active_index) {
        bool found = false;
        for (elg_uint32 current_id_index = 0;current_id_index < current_count;++current_id_index) {
