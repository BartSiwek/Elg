...
PoolManager<Loader, PoolSize>::PoolManager(const Elg::AssetManager::AssetDescriptor *asset_descriptors,
                                           elg_uint32 asset_descriptor_count)
    : m_asset_id_count_(0),
  ...
  for (elg_uint32 descriptor_index = 0;descriptor_index < asset_descriptor_count;++descriptor_index) {
  ...
  // Allocate the memory
  m_asset_id_hashes_ = ElgNew elg_uint32[m_asset_id_count_];
  m_filenames_ = ElgNew std::string[m_asset_id_count_];
  m_internal_id_to_pool_slot_ = ElgNew elg_uint32[m_asset_id_count_];
  ...
  for (elg_uint32 descriptor_index = 0;descriptor_index < m_asset_id_count_;++descriptor_index) {
    ELG_STATIC_ANALYSIS_ASSUME(descriptor_index < asset_descriptor_count);  // Tell static code analysis that we are ok
  ...
  for (elg_uint32 internal_id_index = 0;internal_id_index < PoolSize;++internal_id_index) {
    m_internal_id_to_pool_slot_[internal_id_index] = PoolSize;
  }
  for (elg_uint32 pool_slot_index = 0;pool_slot_index < PoolSize;++pool_slot_index) {
    m_pool_slot_to_internal_id_[pool_slot_index] = static_cast<elg_uint64>(0);
  }

  // Initialize the rest of the members [asset managment]
  for (elg_uint32 pool_index = 0;pool_index < PoolSize;++pool_index) {
...
template<typename Loader, elg_uint32 PoolSize>
elg_uint32 PoolManager<Loader, PoolSize>::Get(elg_uint32 id_hash) {
  elg_uint32* location = std::lower_bound(m_asset_id_hashes_, m_asset_id_hashes_ + m_asset_id_count_, id_hash);
  elg_uint32 offset = (location - m_asset_id_hashes_);
  ASSERT(offset != m_asset_id_count_, "Requested asset with unknwon id");

  if (m_internal_id_to_pool_slot_[offset] == PoolSize) {
    // Get a free slot
    elg_uint32 pool_slot = FreeSlot();
...
template<typename Loader, elg_uint32 PoolSize>
typename Loader::loaded_type* PoolManager<Loader, PoolSize>::GetData(elg_uint32 internal_id) {
...
template<typename Loader, elg_uint32 PoolSize>
elg_uint32 PoolManager<Loader, PoolSize>::FreeSlot() {
  // Try to locate a free slot (it will have usage time of 0) or a slot with earliest usage time -> just use min
  elg_uint32 min_index = 0;
  for (elg_uint32 slot_index = 1;slot_index < PoolSize;++slot_index) {
...
template<typename Loader, elg_uint32 PoolSize>
void PoolManager<Loader, PoolSize>::LoadIntoSlot(elg_uint32 internal_id, elg_uint32 slot) {
...
template<typename Loader, elg_uint32 PoolSize>
void PoolManager<Loader, PoolSize>::UpdateUsageTime(elg_uint32 internal_id) {
  m_pool_usage_[m_internal_id_to_pool_slot_[internal_id]] = Elg::Clock::GameClock::Now();
}

template<typename Loader, elg_uint32 PoolSize>
bool PoolManager<Loader, PoolSize>::HasNoDuplicates(const AssetDescriptor** descriptors, elg_uint32 count) {
  bool is_unique = true;
  for (elg_uint32 descriptor_index = 1;descriptor_index < count;++descriptor_index) {
