...
Asset<AssetManager<AssetManagerDescription>, AssetType> AssetManager<AssetManagerDescription>::Get(elg_uint32 id_hash) {
  // Static asserts
  ...
  elg_uint32 asset_internal_id = m_description_.GetAssetLoader<AssetTypeIndex>().Get(id_hash);
  return Asset<AssetManager<AssetManagerDescription>, AssetType>(this, asset_internal_id);
}

template<typename AssetManagerDescription> template<typename AssetType>
AssetType* AssetManager<AssetManagerDescription>::GetData(elg_uint32 internal_id) {
  // Static asserts
  enum { AssetTypeIndex = AssetManagerDescription::AssetLoaderIndex<AssetType>::value };
  STATIC_ASSERT(AssetTypeIndex != -1, "Unknown asset type");
