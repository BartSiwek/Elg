...
#include "Elg/Utilities/Preprocessor/repetition/enum.hpp"
...
#define ELG_ASSET_MANAGER_DESCRIPTION_HEADER()                                                                        \
  ...
  template<typename AssetLoaderType> struct AssetLoaderIndex {                                                        \
    enum { value = -1 };                                                                                              \
                                                                                                                      ...
                                                                                                                      \
  template<elg_uint16 Index> struct AssetLoaderType;                                                                  \
                                                                                                                      \
  template<elg_uint16 Index> typename AssetLoaderType<Index>::type& GetAssetLoader();                                 \
  template<elg_uint16 Index> const typename AssetLoaderType<Index>::type& GetAssetLoader() const;
