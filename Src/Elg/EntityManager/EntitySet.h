...
class EntitySet {
 public:
  // Consts
  static const elg_uint32 MaxEntityCount = EntityManagerType::MaxEntityCount;

  // Constructors
  explicit EntitySet(const EntityManagerType* owner)
    : m_entity_manager_(owner), m_current_count_(owner->GetActiveEntityCount()) {
    const elg_uint32* entities_source = owner->GetActiveEntities();
    memcpy(m_entities_, entities_source, m_current_count_ * sizeof(*entities_source));
  }

  EntitySet(const EntityManagerType* owner, const elg_uint32* entities, elg_uint32 count)
  ...
  elg_uint32 operator[](elg_uint32 index) const {
  ...
  template<typename T>
  EntitySet& Filter() {
    elg_uint32 entity_index = 0;
    elg_uint32 insertion_index = 0;
  ...
  const elg_uint32* GetEntities() const {
    return m_entities_;
  }

  elg_uint32* GetEntities() {
    return m_entities_;
  }

  elg_uint32 GetCount() const {
 ...
 private:
  ...
  elg_uint32 m_entities_[MaxEntityCount];
  elg_uint32 m_current_count_;
};
