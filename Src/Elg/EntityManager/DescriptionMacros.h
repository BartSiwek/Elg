...
#include "Elg/Utilities/Preprocessor/cat.hpp"
#include "Elg/Utilities/Preprocessor/repetition/repeat.hpp"
...
#define ELG_ENTITY_MANAGER_DESCRIPTION_HEADER(ENTITY_COUNT)                                                           \
  static const elg_uint32 MaxEntityCount = (ENTITY_COUNT);                                                            \
                                                                                                                      \
  ...
  template<typename ComponentType> struct ComponentMaxCount;                                                          \
                                                                                                                      \
  template<elg_uint16 Index> struct ComponentPoolType;                                                                \
                                                                                                                      \
  template<elg_uint16 Index> typename ComponentPoolType<Index>::type& GetComponentPool();                             \
  template<elg_uint16 Index> const typename ComponentPoolType<Index>::type& GetComponentPool() const;


#define ELG_ENTITY_MANAGER_DESCRIPTION_COMPONENT(COMPONENT_INDEX, COMPONENT_COUNT, ...)                               \
  static const elg_uint16 BOOST_PP_CAT(MaxComponentInstances, COMPONENT_INDEX) = (COMPONENT_COUNT);                   \
                                                                                                                      \
...
#define ELG_ENTITY_MANAGER_DESCRIPTION_FOOTER(COMPONENT_COUNT)                                                        \
  static const elg_uint16 ComponentCount = (COMPONENT_COUNT);                                                         \
                                                                                                                      \
  void DestroyAllComponents(elg_uint32 entity_id_index) {                                                             \
