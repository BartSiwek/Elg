...
template<elg_int64 N, elg_int64 D = 1>
struct Ratio {
  static_assert(D != 0, "Denominator cannot be zero");
  static_assert(N >= -static_cast<elg_int64>(0x7fffffffffffffff) &&
                D >= -static_cast<elg_int64>(0x7fffffffffffffff), "Out of range");

  // Note: N * sign(N) == abs(N)
  static const elg_int64 Num = Sign<D>::value * N / Gcd<N, D>::value;

  static const elg_int64 Den = Abs<D>::value / Gcd<N, D>::value;

  typedef Ratio<N, D> type;
};

template<elg_int64 N, elg_int64 D>
const elg_int64 Ratio<N, D>::Num;

template<elg_int64 N, elg_int64 D>
const elg_int64 Ratio<N, D>::Den;
...
template<typename Lhs, typename Rhs,
         elg_int64 LhsSign = Sign<Lhs::Num>::value, elg_int64 RhsSign = Sign<Rhs::Num>::value>
struct RatioSignBasedCompare;

// Ratio inverse compare
template<elg_int64 LhsDen, typename Rhs>
struct RatioInverseCompare<Ratio<0, LhsDen>, Rhs> {
  static const bool value = true;
};

template<typename Lhs, elg_int64 RhsDen>
struct RatioInverseCompare<Lhs, Ratio<0, RhsDen>> {
  static const bool value = false;
};

template<elg_int64 LhsDen, elg_int64 RhsDen>
...
struct RatioContinousFractionCompare {
 private:
  static const elg_int64 LhsIntegral = Lhs::Num / Lhs::Den;
  static const elg_int64 LhsRest = Lhs::Num % Lhs::Den;
  static const elg_int64 RhsIntegral = Rhs::Num / Rhs::Den;
  static const elg_int64 RhsRest = Rhs::Num % Rhs::Den;
...
struct RatioAdd {
 private:
  ...
  static const elg_int64 M = Gcd<Lhs::Den, Rhs::Den>::value;
  static const elg_int64 N = Add< Multiply<Lhs::Num, (Rhs::Den / M)>::value,
  ...
  static const elg_int64 K = Gcd<N, M>::value;

 public:
  typedef Ratio<N / K, Multiply<Lhs::Den / K, Rhs::Den / M>::value> type;

  static const elg_int64 Num = type::Num;
  static const elg_int64 Den = type::Den;
};

template<typename Lhs, typename Rhs>
const elg_int64 RatioAdd<Lhs, Rhs>::Num;

template<typename Lhs, typename Rhs>
const elg_int64 RatioAdd<Lhs, Rhs>::Den;
...
template<typename Lhs, typename Rhs>
const elg_int64 RatioSubtract<Lhs, Rhs>::Num;

template<typename Lhs, typename Rhs>
const elg_int64 RatioSubtract<Lhs, Rhs>::Den;
...
struct RatioMultiply {
 private:
  static const elg_int64 LhsNumRhsDenGcd = Gcd<Lhs::Num, Rhs::Den>::value;
  static const elg_int64 RhsNumLhsDenGcd = Gcd<Rhs::Num, Lhs::Den>::value;
...
template<typename Lhs, typename Rhs>
const elg_int64 RatioMultiply<Lhs, Rhs>::Num;

template<typename Lhs, typename Rhs>
const elg_int64 RatioMultiply<Lhs, Rhs>::Den;
...
template<typename Dividend, typename Divisor>
const elg_int64 RatioDivide<Dividend, Divisor>::Num;

template<typename Dividend, typename Divisor>
const elg_int64 RatioDivide<Dividend, Divisor>::Den;
